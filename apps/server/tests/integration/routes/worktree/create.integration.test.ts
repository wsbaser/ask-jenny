...
import { ASK_JENNY_INITIAL_COMMIT_MESSAGE } from '@/routes/worktree/common.js';
...
describe('worktree create route - repositories without commits', () => {
  ...
  async function initRepoWithoutCommit() {
    repoPath = await fs.mkdtemp(path.join(os.tmpdir(), 'ask-jenny-no-commit-'));
  ...
  it('creates an initial commit before adding a worktree when HEAD is missing', async () => {
    ...
    expect(latestMessage.trim()).toBe(ASK_JENNY_INITIAL_COMMIT_MESSAGE);
  });
});

...
import type { ContextFilesResult, ContextFileInfo } from '@ask-jenny/utils';
 ...
 * (Matches the format used in @ask-jenny/utils/context-loader.ts)
 */
...
export async function getCustomSubagents(
  ...
  projectPath?: string
): Promise<Record<string, import('@ask-jenny/types').AgentDefinition> | undefined> {
...
export interface ActiveClaudeApiProfileResult {
  ...
  credentials: import('@ask-jenny/types').Credentials | undefined;
}
...
export interface ProviderByModelIdResult {
  ...
  modelConfig: import('@ask-jenny/types').ProviderModel | undefined;
  /** Credentials for API key resolution */
  credentials: Credentials | undefined;
    ...
    for (const provider of providers) {
      ...
      if (modelConfig) {
        ...
        if (modelConfig.mapsToClaudeModel) {
          ...
          const { resolveModelString } = await import('@ask-jenny/model-resolver');
...
export async function getAllProviderModels(
  ...
  Array<{
    providerId: string;
    providerName: string;
    model: import('@ask-jenny/types').ProviderModel;
  }>
> {
  try {
    const globalSettings = await settingsService.getGlobalSettings();

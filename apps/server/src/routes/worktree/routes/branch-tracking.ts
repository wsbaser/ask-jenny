 ...
 *
 * Tracks active branches in .ask-jenny so users
 * can switch between branches even after worktrees are removed.
 */

import * as secureFs from '../../../lib/secure-fs.js';
import path from 'path';
import { getBranchTrackingPath, ensureAskJennyDir } from '@ask-jenny/platform';
...
async function saveTrackedBranches(projectPath: string, branches: TrackedBranch[]): Promise<void> {
  ...
  const filePath = path.join(askJennyDir, 'active-branches.json');

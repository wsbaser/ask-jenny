 ...
 *
 * Manages Cursor CLI configuration stored in .ask-jenny/cursor-config.json
 */

import * as fs from 'fs';
import * as path from 'path';
import { getAllCursorModelIds, type CursorCliConfig, type CursorModelId } from '@ask-jenny/types';
 ...
 * Config location: .ask-jenny/cursor-config.json
 */
export class CursorConfigManager {
  ...
  constructor(projectPath: string) {
    // Use getAskJennyDir for consistent path resolution
    this.configPath = path.join(getAskJennyDir(projectPath), 'cursor-config.json');
